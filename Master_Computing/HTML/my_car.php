...
if(!$con){
    ...
    if(isset($_SESSION['user_id'])){
        ...
        $query_shopping = "SELECT * FROM `products` INNER JOIN `shopping_car` WHERE products.product_id = shopping_car.fk_id_product AND $id = shopping_car.fk_id_user";
        $search_shopping = mysqli_query ($con,$query_shopping);
    }else{
        ...
        <div class="products-container">
            ...
            <div class="main-orders">
                <table class="table table-bordered table-myproducts">
                    <?php while($row = mysqli_fetch_array($search_shopping, MYSQLI_ASSOC))
                        {
                            $img_route = "../".$row['product_img'];
                            $route_delete = "../PHP/delete_shopping_car.php?id=".$row['product_id'];
                    ?>
                        <tr class="table-tr">
                            <td>
